...
- **Сайдбар-навігація** (`AppSidebar.vue`, тип `NavTarget`): на десктопі — sticky-рейка `.app-rail` зліва (сайдбар + `DailyGoalPanel.rail-daily`; дубль цілі в side-column має клас `.mobile-daily` і видимий лише ≤620px — стан singleton, тож синхронні), на мобільному (≤620px) — рейка `display: contents`, сайдбар стає фіксованим нижнім таб-баром (`.app-body.with-nav` дає padding-bottom, тости підняті над баром). Пункти: Головна, Дрил, Читання, Спідран, Пари, Екзамен, Досягнення; локи (борг SRS, екзамен раз на тиждень) показуються 🔒 і йдуть через ті самі `start*`-хендлери з тостами. Декор сайдбара: брендова шапка з медальйоном, неоновий градієнтний бордер, SVG-сценка внизу (день у світлій темі / ніч у темній, перемикання через `[data-theme='dark']` + media-гард) і CSS-пелюстки; іконки в кольорових чипах (акцент на пункт), активний пункт — градієнтна пігулка; бейдж на «Тренуванні» = `debtCount` (прострочені SRS). На десктопі рейка ховається в сесійних view (`rail-only-mobile` на `.app-rail`), на мобільному нижній таб-бар видимий **завжди**. Приховування `rail-daily`/`mobile-daily` у base.css — селекторами з трьох класів, щоб перебити scoped `.daily-panel[data-v]`. Кнопки «Тренувати кану»/«Почати читання» живуть у `TextInputPanel` (емити `start-drill`/`start-reading`), не в hero.
...
- **Джерело** (`drillMode` + `useDrillSource`): _які_ кани — весь текст / SRS на сьогодні / слабкі / плутанини / словник N5 / числа / мій словник / кандзі N5 / набори.
- **Словесні джерела — спільний шар `data/wordSources.ts`** (`WordEntry`, `orderByUrgency`, лукапи): режими `vocab`/`numbers`/`custom`/`kanji` (`isWordSource` у useDrillSource) віддають слова через повноширинний пробіл (першими — слова з найтерміновішою каною), дека форсує чанк «ціле слово» (`isWordMode`), картки показують `currentTranslation` у фідбеку (він підтягується і в текстовому режимі при шматку «Слово» — `isWholeWordChunk`, для слів, що є у словниках). `translationFor` індексує і катакана-, і хіраґана-форму (kuromoji зводить читання до хіраґани); власні слова перекривають вбудовані. Слово-рівневі результати **не** пишуться у `kana-stats` (інакше слова зламали б режим «слабкі»).
  - **Словник N5** — `data/vocabulary.ts` (`vocab`). **Числа і час** — `data/numbers.ts` (`numbers`): числівники/години/дні тижня/дати/лічильники, одне канонічне читання (recognition показує кану, dictation її озвучує — неоднозначності читань немає).
  - **Мій словник** (`custom`): парсер `utils/customVocab.ts` («кана = переклад» / CSV, повертає `{entries, errors}` з номерами рядків), стейт `useCustomVocab` (ключ `kana-custom-vocab`, реєструє слова через `setCustomWords`), редактор-textarea у KanaDrill при виборі джерела.
  - **Кандзі N5** (`kanji`, `data/kanjiN5.ts`): `{display: '山', kana: 'やま', translation}`, читання унікальні. Джерело подає ЧИТАННЯ; гліф дека бере з **окремого** індексу `kanjiWordFor` (читання збігаються зі словами словника — спільний індекс зробив би показ гліфа залежним від порядку реєстрації). Recognition показує кандзі замість кани (кана — у фідбеку), dictation додає кандзі у фідбек, анаграма працює з коробки (промпт = кандзі+переклад). `choice`/`writing` для кандзі сховані (`KANJI_UNSUPPORTED` у KanaDrill + watcher коєрсить формат на recognition).
- **Мнемоніки** (`data/mnemonics.ts`): укр. асоціації форми для всіх 46+46 базових кан; `mnemonicFor` знімає дакутен через NFD (が → образ か + примітка). Показ: recognition — при `isWeak` у пре-фідбек cues і завжди у wrong-фідбеку; choice — у wrong-фідбеку під контрастом. Лише single-kana картки.
...
- **Формат** (`useDrillDeck.format`, persisted): _як_ перевіряти. Спільний стан (навігація, stats/SRS, підсумок) — у **`useDrillDeck`**; формати — тонкі компоненти, що отримують `deck` пропом і кличуть `answer*`/`skip`/`restart`:
  - `recognition` → `DrillRecognitionCard` (бачиш кану/кандзі → ромадзі `answerRomaji` / голос `answerVoice`)
  - `dictation` → `DrillDictationCard` (TTS `speakKana` → ввід ромадзі АБО каною з екранної ґодзюон-клавіатури `KanaKeyboard.vue`: преф `dictationInput` у `kana-drill-prefs`, складання дакутенів/малих кан — `utils/kanaCompose.ts`, сабміт через `answerVoice`)
  - `choice` → `DrillChoiceCard` (ромадзі/звук → 4 плитки з `buildDistractors`; хибний тап → `confusedWith` + контраст; на десктопі гарячі клавіші 1–4 з бейджами)
  - `anagram` → `DrillAnagramCard` (переклад/ромадзі+🔊 → збери чанк із перемішаних плиток, `utils/anagram.ts shuffleTiles` гарантує не-тотожний порядок; авто-сабміт `answerKana`; чанк ≥2 у текстовому режимі, ціле слово у словесних)
...
- Дрил: `deck.skip(count)` пропускає кілька карток (`SkipControl.vue` — спліт-кнопка «Пропустити» + меню 3/5/10, спільна для всіх форматів). У recognition/dictation фокус повертається в інпут на **кожній** зміні картки, а форма лишається змонтованою під час correct-паузи (`v-if="lastOutcome !== 'wrong'"`) — інакше на мобільному клавіатура закривається і інпут доводиться тапати знову (iOS не відкриє її programmatic focus-ом поза жестом).
...
- **Ачивки** (`utils/achievements.ts` — каталог + `evaluate(snapshot)`; `useAchievements.sync()` — diff проти `kana-achievements`). `useProgressSnapshot.build()` збирає снапшот зі stores. `App.syncAchievements()` кличеться на межах сесій/ігор/mount і тостить нові. Повний каталог — сторінка `#/achievements` (`AchievementsPage.vue`, прогрес через опційне `progress?(s)`); вхід — пункт «Досягнення» в сайдбарі. Іконки — SVG-медальйони `AchievementIcon.vue` (гліфи за id, emoji-фолбек для нових; `Toast.achievementId` рендерить медальйон у тості). Анти-ачивки з `shame: true` («Зала ганьби», попелястий варіант) живляться `maxConfusionCount` і `longestHesitationMs` (max-вагання пише deck у `drill:hesitation`, ігноруючи 60с+ і таймаути).
...
- **Звуки** (`utils/sfx.ts` — ноти/резолв, тестовані; `composables/useSfx.ts` — Web Audio синтез, singleton-тумблер `kana-sfx`, кнопка 🔊 у топбарі): `correct`/`wrong` у деці дрила і спринті + матч у «Парах», `combo` на згорянні комбо, `fanfare` на ачивках/денній цілі/рекордах, `finish` на кінці сесії/гри. Без аудіофайлів; збої Web Audio мовчки ковтаються. Екзамен навмисно без звуків.
...
- **Зникаючий текст читання** (`utils/readingFocus.ts`, ключ `kana-reading-hide`): `fade` блюрить прочитане, `flash` показує 8-канне вікно на 2 с і маскує (тікер `now` реюзається), `rsvp` («Фокус») — видиме лише ковзне 8-канне вікно попереду курсора, без блимань. Пунктуація завжди видима.
