...
- Дрил: `deck.skip(count)` пропускає кілька карток (кнопки ×3/×5/×10 у всіх форматах). У recognition/dictation фокус повертається в інпут на **кожній** зміні картки, а форма лишається змонтованою під час correct-паузи (`v-if="lastOutcome !== 'wrong'"`) — інакше на мобільному клавіатура закривається і інпут доводиться тапати знову (iOS не відкриє її programmatic focus-ом поза жестом).
