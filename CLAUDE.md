...
- **Сайдбар-навігація** (`AppSidebar.vue`, тип `NavTarget`): на десктопі — sticky-rail зліва, на мобільному (≤620px) — фіксований нижній таб-бар (`.app-body.with-nav` дає padding-bottom, тости підняті над баром). Пункти: Головна, Дрил, Читання, Спідран, Пари, Екзамен, Досягнення; локи (борг SRS, екзамен раз на тиждень) показуються 🔒 і йдуть через ті самі `start*`-хендлери з тостами. Іконки в кольорових чипах (акцент на пункт), активний пункт — градієнтна пігулка; бейдж на «Тренуванні» = `debtCount` (прострочені SRS). На десктопі rail ховається в сесійних view (`rail-only-mobile`), на мобільному нижній таб-бар видимий **завжди**. Кнопки «Тренувати кану»/«Почати читання» живуть у `TextInputPanel` (емити `start-drill`/`start-reading`), не в hero.
