...
- **Сайдбар-навігація** (`AppSidebar.vue`, тип `NavTarget`): на десктопі — sticky-rail зліва, на мобільному (≤620px) — фіксований нижній таб-бар (`.app-body.with-nav` дає padding-bottom, тости підняті над баром). Пункти: Головна, Дрил, Читання, Спідран, Пари, Екзамен, Досягнення; локи (борг SRS, екзамен раз на тиждень) показуються 🔒 і йдуть через ті самі `start*`-хендлери з тостами. Видимий лише на `setup`/`result`/`achievements` — сесійні view лишаються фокус-режимами. Кнопки ігор/екзамену з hero і тизер-панель ачивок (AchievementsPanel) прибрані на користь сайдбару.
